...
| 1 — Scaffold (Attempt 3) | 2026-05-01 | **PASS** | Branch `fix/p1-drop-platformio`. rev4 contract: PlatformIO dropped entirely; single canonical build path (ESP-IDF v5.5.4); 4-step smoke gate. All four steps green: pre-commit (6 hooks), `idf.py build` (binary `rak3112_rs485_node.bin` 183 328 B, `IDF_VER=v5.5.4` clean, SHA-256 `a2f3d604…fe5116`), gitleaks (11 commits / 243 KB / no leaks), layout check. ESP-IDF stash restored cleanly post-tag. Tagged `phase-1-scaffold-green` with toolchain version annotation. Phase 1 closes; advance gated on user review. |
