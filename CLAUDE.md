...
#### 7a — Resilience: Task Watchdog + brownout + reset-cause log · **GREEN (HIL) 2026-06-25**
...
#### 7b — Power: light-sleep duty-cycling between samples · **IMPLEMENTED 2026-06-25 (opt-in, default OFF) · cycling + current DEFERRED to DC/battery bench**
...
#### 7c — OTA: dual-slot partitions + update with rollback · **GREEN (HIL) 2026-06-25 · network transport deferred (OQ-10)**
...
#### 7d — Provisioning: NVS-stored credentials & runtime config · **GREEN (HIL) 2026-06-25**
...
| 7a — Resilience (watchdog + brownout + reset-cause) | 2026-06-25 | **PASS (HIL)** | Branch `phase/p7-production-hardening`. `app_main.c`: `log_boot_diagnostics()` (reset reason via `esp_reset_reason` + persistent NVS `faultlog/bootcnt`), `field_wdt_arm()` (subscribe field-app task to TWDT, reconfigure to `CONFIG_APP_TASK_WDT_TIMEOUT_S`, panic-on-starve) and **fed-sleep** `wdt_fed_delay()` (≤2 s chunks feed the dog through the long inter-sample sleep; a hang in the work section still trips it). `sdkconfig.defaults`: `CONFIG_ESP_TASK_WDT_INIT=y`, brownout pinned `CONFIG_ESP_BROWNOUT_DET=y` + `LVL_SEL_7` (IDF S3 default — not raised, avoids RT6160 spurious resets). Kconfig: `APP_TASK_WDT_TIMEOUT_S` (def 15), debug `APP_DEBUG_WDT_STARVE`(+`_AFTER`) gate aid; repro profile `sdkconfig.defaults.wdt-test`. **HW pre-flight:** single port `/dev/cu.usbmodem1301`, MAC `3c:dc:75:6f:85:dc` (project board; H1 confirmed by USB enum). **Gate PASS both halves on the project board:** (1) starve build → `task_wdt: Task watchdog got triggered` → reboot `rst:0xc` → next boot `=== boot #N — reset reason: task-WDT (6) ===`, NVS bootcnt climbed #2→#3→#4; (2) clean sim build **~11.5-min soak** (~49 uplinks every 10 s), **0** TWDT / **0** brownout / **0** reboots, `esp_log` ms timer monotonic 15.9 s→690 s (no zeroing = no reset). Default build green (binary `0x4e140`, 70% app-partition free), `IDF_VER=v5.5.4-dirty` (local PARLIO mod, off-compile-path, Footnote 1). RUNBOOK "Phase 7a" lesson: a watchdog on a long-interval sampler only works if the *intended* sleep keeps it fed (chunked feed). Carries to 7b: light-sleep changes feed cadence → revisit timeout + feed-on-wake. Default firmware re-flashed to leave the bench clean. |
| 7d — Provisioning (NVS creds + runtime config) | 2026-06-25 | **PASS (HIL)** | Branch `phase/p7-production-hardening`. Headline: **no compiled-in secrets** — OTAA creds + Modbus config move to NVS namespace `prov`; compiled `lora_credentials.h` is fallback only. `lora.cpp`: NVS deveui/joineui(u64)+appkey(blob) take priority; `lora_is_provisioned()` false only when NVS empty AND compiled key all-zero → node waits, no bogus join. `app_main.c::load_field_cfg`: NVS `dev/baud/par/unit/intv` override Kconfig; field loop branches on device at **runtime** (both MFM384+RS-FSJT compiled). `provisioning.c` (+Kconfig `APP_PROVISIONING_CONSOLE=y`, `console` component): USB-Serial-JTAG REPL `prov-lorawan/-modbus/-show/-clear/-done` on its own task, **available in field mode** too, writes only `prov` ns → **nonces/session preserved** (no DevNonce regression; hence a console, not a whole-partition `parttool` overwrite). Host `tools/provision_nvs.py` drives it from `firmware/.env` (AppKey never echoed — linenoise dumb-mode + tool redaction); `.env.example` gains `MODBUS_DEVICE`/`SAMPLE_INTERVAL_S`. **HIL gate all 3 PASS** (project board, factory placeholder-creds image): (1) empty NVS → `AWAITING PROVISIONING`, no `OTAA:`/join; (2) tool writes NVS (`dev=1/4800/20`) → boot `OTAA: DevEUI=3cdc75fffe6f85dc [creds:NVS]` + `session restored` + `field app: REAL Modbus RS-FSJT … 20s [cfg:NVS]` (placeholder compiled key can't join → `uplink OK` proves NVS creds); (3) re-provision in field mode (`dev=0/9600/60`) on the **same binary** → `field app: REAL Modbus MFM384 … 60s [cfg:NVS]` = config change, no rebuild. Default real-creds image re-flashed (logs `[creds:NVS]` — NVS priority over fallback = intended provisioned state); pristine/placeholder build green (CI-safe). Bench (default) binary SHA-256 `697c7d25045e20dfabde1e7afdcf9a75932a2d064bd9f302c4561f737299196d`, `IDF_VER=v5.5.4-dirty` (local PARLIO mod, off-compile-path, Footnote 1). Lessons (RUNBOOK 7d): provision additively to preserve LoRaWAN nonces; a new Kconfig option needs `rm sdkconfig`/`fullclean` to regenerate (console silently compiled out otherwise — caught via `strings`/`nm`). Remaining Phase 7: 7b (light-sleep, needs current rig), 7c (OTA, needs erase confirm). |
| 7c — OTA (dual-slot partitions + rollback) | 2026-06-25 | **PASS (HIL)** | Branch `phase/p7-production-hardening`. Dual-slot `firmware/partitions.csv` (nvs **kept at 0x9000/0x6000** → 7d provisioning + LoRaWAN nonces survive, **no full erase**): `nvs · otadata · phy · ota_0@0x20000(3M) · ota_1@0x320000(3M)`; `sdkconfig.defaults` adds `CONFIG_PARTITION_TABLE_CUSTOM` + `CONFIG_BOOTLOADER_APP_ROLLBACK_ENABLE=y`. `ota.c/.h` (+`app_update`/`esp_partition`): boot-slot log, `ota_mark_valid()` (cancel pending rollback once healthy), console `ota-status`/`ota-activate <0|1>` (real `esp_ota_set_boot_partition` → PENDING_VERIFY). Kconfig `APP_BUILD_TAG` (A/B) + `APP_DEBUG_OTA_BAD` (aborts before mark-valid; profile `sdkconfig.defaults.ota-bad`). **HW safety:** backed up `0x0..0x110000` (1.11 MB, SHA `94a4082c…`) pre-flash. **HIL gate all 3 PASS** (project board): (1) layout flash → `running slot=ota_0 state=valid tag='A'` + `[creds:NVS]`/`[cfg:NVS]` (provisioning preserved across the layout change); (2) staged build B into ota_1 via `parttool`, `ota-activate 1` → boots ota_1/B pending-verify → `ota_1 marked VALID` → **survived 2 resets**; (3) staged BAD (abort-before-validate) into ota_0, `ota-activate 0` → boots → `abort() … aborting before mark-valid` → **bootloader rolled back** → `running slot=ota_1 state=valid tag='B'`, node recovered (`[creds:NVS]` join + `uplink OK`). Default tag-A re-flashed → clean `running slot=ota_0` production state; binary SHA-256 `5d503aced11a6dba1f0572e0af414b3ac1e0c47eaf75cbf9bb3bde972ec3da11`, `IDF_VER=v5.5.4-dirty` (local PARLIO mod, off-compile-path, Footnote 1). **Network OTA transport deferred (OQ-10):** WiFi/HTTPS then FUOTA — blocked on bench WiFi creds (like 7b's current rig); the partition + rollback foundation is in place. Lessons (RUNBOOK 7c): hold nvs at its offset to avoid a destructive erase; `SDKCONFIG_DEFAULTS` cache stickiness silently produced a BAD "default" build (fullclean/explicit defaults + verify tag before flash); linenoise dumb-mode eats the first scripted command (prime with a throwaway). Phase 7 remaining: **7b** (light-sleep, needs current rig) then **7e** sign-off. |
| 7b — Power (light-sleep duty-cycle) | 2026-06-25 | **IMPLEMENTED · power-gate DEFERRED** | Branch `phase/p7-production-hardening`. Per operator: functional-only this pass, no current target (OQ-12 open). `CONFIG_APP_LIGHT_SLEEP` (Kconfig, **default OFF**): `wdt_fed_delay()` makes each ≤2 s chunk a manual `esp_light_sleep_start()` (RAM retained → LoRaWAN session survives, no re-join); chunking keeps the task-WDT fed; `field_wdt_arm()` sets `idle_core_mask=0` when sleeping (manual light-sleep freezes idle by design). Builds green both ways (off binary `0x5f0b0`). **KEY FINDING:** light-sleep **wedges the node on USB power (VBUS present)** — after cycle 0 even `esptool` can't connect ("No serial data received"); recovery = plug-in-while-holding-SW2(BOOT) → download mode → reflash light-sleep-OFF. Hence default OFF: enable + verify only on a **DC/battery (no-VBUS) bench**, which is also the only setup that measures true average current (OQ-12). **2nd finding:** pyserial default `dtr/rts=True` drives BOOT/EN on the USJ and holds the chip in reset (0 console bytes on a healthy board) — read with `dtr=False,rts=False`. **Verified (light-sleep OFF baseline):** recovered node boots once, OTA layout intact, `[creds:NVS]`, `session restored`, RS-FSJT field app, **2+ cycles + uplinks, monotonic timer** (continuous op + session-preservation). **RS-FSJT real read (Phase 6 follow-up) CLOSED:** read **succeeded** pre-disturbance (`wind=0.00 m/s`, no read-failed — vs Phase 6 silence); went stale after the recovery power-cycles (bench/wiring, re-seat). Default firmware stays light-sleep OFF (proven 7a/7c busy-wait). `IDF_VER=v5.5.4-dirty`. Lessons (RUNBOOK 7b): don't light-sleep with USJ+VBUS (wedges; verify on battery); de-assert DTR/RTS reading a USJ port; recover a wedged board via download-on-plug. **Phase 7 remaining: 7b full verification (battery bench) + 7e sign-off.** |
