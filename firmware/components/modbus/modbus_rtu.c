...
size_t modbus_build_write_single(uint8_t *out, uint8_t slave, uint16_t addr, uint16_t value)
{
    ...
    out[6] = (uint8_t)(crc & 0xFFu); /* CRC low byte first on the wire */
    out[7] = (uint8_t)(crc >> 8);
    return MODBUS_WRITE_SINGLE_LEN;
}
                                                   ...
                                                   uint16_t expect_value, uint8_t *exception_out)
{
    ...
    if (adu[1] == (uint8_t)(MODBUS_FC_WRITE_SINGLE_REGISTER | 0x80u)) {
        if (exception_out != NULL) {
            *exception_out = adu[2];
        }
        return MODBUS_ERR_EXCEPTION;
    }
    if (adu[1] != MODBUS_FC_WRITE_SINGLE_REGISTER) {
        return MODBUS_ERR_FUNC;
    }

    /* Normal response echoes the request verbatim (8 bytes). Confirm the echoed addr/value. */
    if (len != MODBUS_WRITE_SINGLE_LEN) {
        return MODBUS_ERR_BYTECOUNT;
    }
    const uint16_t echo_addr = (uint16_t)(((uint16_t)adu[2] << 8) | (uint16_t)adu[3]);
    const uint16_t echo_value = (uint16_t)(((uint16_t)adu[4] << 8) | (uint16_t)adu[5]);
    if (echo_addr != expect_addr || echo_value != expect_value) {
        return MODBUS_ERR_BYTECOUNT; /* slave echoed a different register/value than requested */
    }
    return MODBUS_OK;
}
                                ...
                                uint16_t qty, const uint16_t *regs)
{
    ...
    const size_t frame = (size_t)9u + 2u * (size_t)qty; /* 7 header + 2*qty data + 2 CRC */
    if (out_cap < frame) {
        return 0;
    }
    ...
    out[4] = (uint8_t)(qty >> 8);
    out[5] = (uint8_t)(qty & 0xFFu);
    out[6] = (uint8_t)(2u * qty); /* byte count */
    for (uint16_t i = 0; i < qty; ++i) {
        out[7u + 2u * i] = (uint8_t)(regs[i] >> 8); /* register hi byte first on the wire */
        out[8u + 2u * i] = (uint8_t)(regs[i] & 0xFFu);
    }

    const size_t body = (size_t)7u + 2u * (size_t)qty;
    const uint16_t crc = modbus_crc16(out, body);
    out[body] = (uint8_t)(crc & 0xFFu); /* CRC low byte first */
    out[body + 1u] = (uint8_t)(crc >> 8);
    return frame;
}
                                                  ...
                                                  uint16_t expect_qty, uint8_t *exception_out)
{
    ...
    if (adu[1] == (uint8_t)(MODBUS_FC_WRITE_MULTIPLE_REGISTERS | 0x80u)) {
        if (exception_out != NULL) {
            *exception_out = adu[2];
        }
        return MODBUS_ERR_EXCEPTION;
    }
    if (adu[1] != MODBUS_FC_WRITE_MULTIPLE_REGISTERS) {
        return MODBUS_ERR_FUNC;
    }

    /* Normal response echoes addr + qty (8 bytes). */
    if (len != MODBUS_WRITE_MULTI_RESP_LEN) {
        return MODBUS_ERR_BYTECOUNT;
    }
    const uint16_t echo_addr = (uint16_t)(((uint16_t)adu[2] << 8) | (uint16_t)adu[3]);
    const uint16_t echo_qty = (uint16_t)(((uint16_t)adu[4] << 8) | (uint16_t)adu[5]);
    if (echo_addr != expect_addr || echo_qty != expect_qty) {
        return MODBUS_ERR_BYTECOUNT;
    }
    return MODBUS_OK;
}
