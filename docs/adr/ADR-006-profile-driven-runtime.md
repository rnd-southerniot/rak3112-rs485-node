...
- **Status:** Proposed (increments 1–4 done; **3 + 4 HW-validated on real MFM384**; increment 5 gated)
- **Date:** 2026-06-24
...
| 3 ✅ | On-target generic reader (`meter_read_profile`) + app wiring (runtime auto-detect: NVS profile → generic path, else compiled) | **done — HW-validated 2026-06-24** on board `3c:dc:75:6f:85:dc`: MFM384 profile (452 B blob via `prov-profile`) drove a live read — `PROFILE-DRIVEN device_byte=0x01, 23 measurands`, V1N=54.67 V (sane, == compiled FC04/CDAB decode), 39 B frame, `uplink OK`, no stale. RS-FSJT profile authored byte-identical to the compiled path (pending sensor power). **Caveat:** the 39 B MFM384 profile frame needs its matching ChirpStack decoder (firmware decoder is the 19 B Phase-6c layout) — CRM-side decoder-from-profile is a follow-up |
| 4 ✅ | Auto-scan-for-ID (`meter_scan_for_unit`; probe configured unit first, else scan 1..`APP_FIELD_SCAN_ID_HI`, persist) | **done — HW-validated 2026-06-24**: configured unit set wrong (9) → `configured unit 9 silent — scanning bus...` → `persisted discovered unit 1` → read MFM384 (V1N 53.5 V), `uplink OK`. Probe match = CRC-valid reply OR Modbus exception |
| 5 (partial) | Validate all 4 profiles + soak. **`dp_simulate()`** (pure, host-tested) drives a **profile-driven simulator** — with `CONFIG_APP_FIELD_SIMULATE` + a provisioned profile, the field app skips the bus/scan, joins, and uplinks synthesized per-profile data (SIMULATED flag), so EEM400/DSE (no meter on hand) validate the blob→encode→fleet-decoder path over LoRaWAN. | each profile decodes sane in ChirpStack; budgets green. **Software half done** (host `dp_simulate` KAT + builds); the ChirpStack decode of a sim EEM400/DSE uplink + a soak are the bench-gated remainder |
